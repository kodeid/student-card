import {HiOutlineMapPin, HiOutlineMagnifyingGlass, HiOutlineEnvelope, HiOutlineBriefcase, HiOutlineBell, HiOutlineFaceSmile, HiOutlineFaceFrown, HiCalendarDays} from 'react-icons/hi2'
...
export default function CardProfile({detail}) {
  return (
    <div className="w-full rounded-lg shadow mb-5 p-10 bg-white flex">
      <div className="flex flex-col gap-3 w-1/3">
        ...
        <div className="flex text-xs">
          ...
          <span className="w-2/3 font-bold pl-3">{formatDate(detail?.batches[detail?.batches.length - 1].startDate)}</span>
        </div>
        <div className="flex text-xs">
          <div className="w-1/3 flex flex-wrap">
            <span className="opacity-70 flex items-start gap-1">
          ...
          <span className="w-2/3 font-bold pl-3">{formatDate(detail?.batches[detail?.batches.length - 1].endDate)}</span>
        </div>
      </div>
